...
def choose_action(qmatrix, currentState, angle_red,angle_yellow, opening_angle, actions, episode, nb_fast_ep):
	# EPSILON = (time.time() - start_time)/300
	# EPSILON = 0
	# if (time.time() - start_time >= 300) :
	# if (episode>= nb_fast_ep):
		# EPSILON = 1
	EPSILON = 1
...
def reward_function(max_speed, max_dist, speed, sum_distance, State, action_index):
	R = -(sum_distance/max_dist) + 1
	rewardtable[State,action_index] = R
	# print("R = %f = -0.5*(%d/%d) - 0.5*(%f/%f) + 1"%(R, speed, max_speed, sum_distance, max_dist) )
	print("Rtable",sparse.csr_matrix(rewardtable))
	return R
...
def poolTable(qmatrix, actions, State, State_index, Sindex, ball_pos, episode, points):
	...
	# Arcsin => angle entre [-pi/2,pi/2] et on veut [0,360]
	...
	action_index,EPSILON = choose_action(qmatrix, State, angle_red, angle_yellow, opening_angle, actions, episode, nb_fast_ep)
	# action_index = 677
	...
	if (nb_first_ep < episode < nb_fast_ep):
		pygame.display.set_caption("Episode %d - Fast until episode %d - Points: %d - epsilon = %0.2f" % (episode, nb_fast_ep,points, EPSILON))
	else: 
		pygame.display.set_caption("Episode %d - Points marqués: %d - epsilon = %0.2f" % (episode,points, EPSILON))
	while loop:
		...
		if (whiteb.speed == 0 and redb.speed == 0 and yellowb.speed == 0):
			R = 0
			nextState = 0
			sum_distance = sqrt((ball_pos[2][0]-ball_pos[1][0])**2+(ball_pos[2][1]-ball_pos[1][1])**2) + sqrt((ball_pos[0][0]-ball_pos[1][0])**2+(ball_pos[0][1]-ball_pos[1][1])**2)
			speed = white_speed
			print("Speed: %d" %speed)
			if (collWR == True and  collWY == True):
				R = reward_function(max_speed,max_dist, speed, sum_distance, State, action_index)
				# R = 1
			...
			if R != 0:
				points += 1
				if (points == max_score):
					print("end with %d episode"%episode) 
					close()
		...
		if (nb_first_ep < episode < nb_fast_ep):
			clock.tick(1e7) # Increase FPS to simulate faster
		else:
			clock.tick(60)
...
red = (203, 67, 53)
green = (40, 180, 99)
# EPSILON = 0.5  # greedy police > 1 - pourcentage de chance de choisir une action aléatoire 
...
max_score = 10
max_speed = 11
max_dist = 2*sqrt((width-2*radius)**2 + (height - 2*radius)**2)
actions = []
for i in np.arange(0.1,max_speed,1):
	for j in np.arange(0,360,4):
		actions.append((i,j))
# actions = [(10,20),(5,25),(7,30)] good example to understand
rewardtable = np.zeros((1000,len(actions)))
